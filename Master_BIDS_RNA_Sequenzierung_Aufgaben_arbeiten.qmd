...
DESeq2, ein statistisches Softwarepaket, verwendet eine eine solche negative Binomialverteilung, die speziell geeignet ist, um die Zählwerte von RNA-Seq-Daten (Count-Daten) zu modellieren.

Unterschiede in Sequenziertiefe und Library-Größe werden durch sogenannte Size Factorskorrigiert. Dadurch sind die Proben vergleichbar. Ausserdem kann DESeq2 die Overdispersion schätzen, wodurch sich die Genauigkeit der p Werte verbessert, welche benötigt werden , um Vergleiche zwischen den einzelnen Medikamentengruppen anstellen zu können. Daher sehen wir uns nun an, wie wir ein DESeq erstellen können
